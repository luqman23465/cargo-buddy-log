  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <Input type="number" placeholder="Load Amount ($)" value={leg.loadAmount || ''} onChange={e => up('loadAmount', Number(e.target.value))} />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <Input type="number" placeholder="Fuel Cost ($)" value={leg.fuelCost || ''} onChange={e => up('fuelCost', Number(e.target.value))} />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <Input type="number" placeholder="Parts Cost ($)" value={leg.sparePartsCost || ''} onChange={e => up('sparePartsCost', Number(e.target.value))} />
      </div>
      ...
      <div className="space-y-2">
        {trips.length === 0 ? (
          ...
          <div key={t.id} className="stat-card">
            <button className="w-full flex items-center justify-between" onClick={() => setExpanded(expanded === t.id ? null : t.id)}>
              <div className="flex items-center gap-4">
                <span className="font-mono text-xs text-muted-foreground">{t.outboundDate}</span>
                <span className="font-medium">{t.outbound.destination}</span>
              ...
              <div className="flex items-center gap-4">
                <span className="font-mono text-sm">${fmt(calcTripRevenue(t))}</span>
                <span className={`font-mono text-sm ${calcTripProfit(t) >= 0 ? 'text-success' : 'text-destructive'}`}>
                  {calcTripProfit(t) >= 0 ? '+' : ''}${fmt(calcTripProfit(t))}
                </span>
            ...
            {expanded === t.id && (
              <div className="mt-4 pt-4 border-t border-border grid grid-cols-2 md:grid-cols-4 gap-4 text-sm animate-fade-in">
                ...
                <div><span className="text-muted-foreground text-xs block">Total Fuel</span><span className="font-mono">${fmt(t.outbound.fuelCost + (t.returnLeg?.fuelCost || 0))}</span></div>
                <div><span className="text-muted-foreground text-xs block">Trip Pay</span><span className="font-mono">${fmt(t.tripPay)}</span></div>
                <div><span className="text-muted-foreground text-xs block">Total Expenses</span><span className="font-mono">${fmt(calcTripExpenses(t))}</span></div>
